...
#plot_ly(x=graphic1[,1],y=graphic1[,2],text=rownames(graphic1),type='scatter',mode="markers",marker=list(color=c("black","blue","red","violet","green")))%>%layout(title="Risk vs. Reward",xaxis=xlab,yaxis=ylab)
...
AMZN%>%Ad()%>%chartSeries(name = 'Amazon (AMZN)')
AMZN%>%chartSeries(TA='addBBands();addBBands(draw="p");addVo();addMACD()',subset='2018/2019', name = 'Amazon (AMZN)')
